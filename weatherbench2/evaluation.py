...
def open_source_files(
  ...
  obs = xr.open_zarr(obs_path, chunks='auto' if (use_dask or by_init) else None, storage_options=_gcs_opts(obs_path) or None)
  forecast = xr.open_zarr(
      ...
      storage_options=_gcs_opts(forecast_path) or None,
  )
...
def open_forecast_and_truth_datasets(
  ...
  if eval_config.evaluate_climatology:
    _clim_path = data_config.paths.climatology
    _clim_opts = {'token': 'anon'} if str(_clim_path).startswith('gs://') else {}
    climatology = xr.open_zarr(_clim_path, storage_options=_clim_opts or None)
