...
def main(argv: list[str]) -> None:
  ...
  try:
    def _gcs_opts(path):
      return {'token': 'anon'} if str(path).startswith('gs://') else {}

    if LSM_DATASET.value:
      land_sea_mask = xr.open_zarr(LSM_DATASET.value, storage_options=_gcs_opts(LSM_DATASET.value) or None)['land_sea_mask'].compute()
    else:
      land_sea_mask = xr.open_zarr(OBS_PATH.value, storage_options=_gcs_opts(OBS_PATH.value) or None)['land_sea_mask'].compute()
    land_regions = {
  ...
  _clim_opts = {'token': 'anon'} if str(CLIMATOLOGY_PATH.value).startswith('gs://') else {}
  climatology = xr.open_zarr(CLIMATOLOGY_PATH.value, storage_options=_clim_opts or None)
